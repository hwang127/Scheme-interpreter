...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    elif  env.lookup(first):
        #print('DEBUG: ',env.lookup(first))
        proce=env.lookup(first)
    def eval(expr):
    #    print('DEBUG: ',expr)
        return scheme_eval(expr,env)
    #print('DEBUG: ','eval rest')
    #if rest.first:
    #print('DEBUG: ','is procedure',check_procedure(proce))
    operand=rest.map(eval)
    #print('DEBUG: ',scheme_procedurep(proce))
    try:
            #check_procedure(proce)
        # BEGIN PROBLEM 5
            #operand=Scheme_eval(rest)
        #print('DEBUG: ',proce,operand)
        return complete_apply(proce,operand,env)
    except:
        raise SchemeError('wrong procedure and operand')
...
def scheme_apply(procedure, args, env):
    ...
    if isinstance(procedure, BuiltinProcedure):
        ...
        '''
        #print('DEBUG: ','apply', args)
        #print('DEBUG: ','apply', isinstance(procedure,LambdaProcedure),procedure.formals)
        if isinstance(procedure,MuProcedure):

            new_env = procedure.make_call_frame(args, env)
        else:
            new_env = procedure.make_call_frame(args, procedure.env)
        #print('DEBUG: ','new frame args',new_env.bindings)
...
def eval_all(expressions, env):
    ...
    # BEGIN PROBLEM 8
    #print('DEBUG: ','eval all',expressions)
    if expressions==nil:
        return None
    if expressions.second==nil:
        #print('DEBUG: ','last',expressions.first)
        return scheme_eval(expressions.first, env)
    else:
        scheme_eval(expressions.first,env)
        #print('DEBUG: ',expressions.second,expressions.second.second)
    ...
    def make_child_frame(self, formals, vals):
        ...
        def bind(frame,formals,vals):
            ...
            if formals==nil or vals==nil:
                raise SchemeError('parameter incorrect')
            a.define(formals.first,vals.first)
            bind(frame,formals.second,vals.second)
        bind(a,formals,vals)
        return a
...
class BuiltinProcedure(Procedure):
    ...
    def apply(self, args, env):
        ...
        try:
            ...
            raise SchemeError('given incorrect number of arguments.')
        # END PROBLEM 4
...
class LambdaProcedure(Procedure):
    ...
    def make_call_frame(self, args, env):
        ...
        # BEGIN PROBLEM 12
        #print('DEBUG: ','new frame',args)

        return env.make_child_frame(self.formals,args)
        #print('DEBUG: ','new frame made')
        # END PROBLEM 12
...
def do_begin_form(expressions, env):
    ...
    #print('DEBUG: ',eval_all(expressions, env))
...
def do_cond_form(expressions, env):
    ...
    while expressions is not nil:
        ...
        #a=scheme_eval(expressions.first.first,env)
        #print('DEBUG: ','a=',scheme_eval(a,env))
        if scheme_truep(test):
            # BEGIN PROBLEM 14
            #print('DEBUG: ','condition=true','expression=',expressions.first.second)
...
def make_let_frame(bindings, env):
    ...
    check_formals(formals)
    def get_values(bindings):
        if bindings==nil:
            return nil
        check_form(bindings.first,2,2)
        #print('DEBUG: ',bindings.first.second.first )
        return Pair(scheme_eval(bindings.first.second.first,env),get_values(bindings.second))
    vals=get_values(bindings)
